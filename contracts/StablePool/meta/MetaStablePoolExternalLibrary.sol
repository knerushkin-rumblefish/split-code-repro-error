...
import "../StableOracleMathLibrary.sol";
 ...
 */
contract MetaStablePoolExternalLibrary is StablePool, PoolPriceOracle {
...
//         If the contract is paused, the oracle is not updated to avoid extra calculations and reduce potential errors.
    ...
    function _updateOracle(
        ...
        if (miscData.oracleEnabled() && block.number > lastChangeBlock) {
            (int256 logSpotPrice, int256 logBptPrice) = StableOracleMathLibrary.calcLogPrices(
                currentAmp,
                balance0,
                balance1,
